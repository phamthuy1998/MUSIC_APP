...
class HistorySongRepositoryImpl() : HistorySongRepository {
    ...
    override fun getHistoryListSong(): ResultData<ArrayList<Song>> {
        ...
        val query = currentUser()?.uid?.let { databaseRef()?.child(HISTORY)?.child(it) }
